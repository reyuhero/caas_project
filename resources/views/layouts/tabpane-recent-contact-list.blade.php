            ...
            <a href="#">
                <div class="media">
                    @if (Cache::has('is_online' . $user->id))
                        <div class="chat-user-img online align-self-center ms-3">
                            <img src="{{ $user->avatar }}" class="rounded-circle avatar-xs" alt="">
                            <span class="user-status"></span>
                        </div>
                    @else
                        <div class="chat-user-img away align-self-center ms-3">
                            <img src="{{ $user->avatar }}" class="rounded-circle avatar-xs" alt="">
                            <span class="user-status"></span>
                        </div>
                    @endif
                    ...
                    <div class="media-body overflow-hidden">
                        ...
                        @if ($user->file)
                            @if (strtolower(pathinfo($user->file, PATHINFO_EXTENSION)) == 'jpg' || strtolower(pathinfo($user->file, PATHINFO_EXTENSION)) == 'png' || strtolower(pathinfo($user->file, PATHINFO_EXTENSION)) == 'gif')
                                <p class="chat-user-message text-truncate mb-0"><i class="ri-image-fill align-middle ms-1"></i> {{ $user->message }}</p>
                            @else
                                <p class="chat-user-message text-truncate mb-0"><i class="ri-file-text-fill align-middle ms-1"></i> {{ $user->message }}</p>
                            @endif
